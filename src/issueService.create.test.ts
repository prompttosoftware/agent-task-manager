...
// Declare mock functions for the mocked database module and uuid
const mockLoadDatabaseFunction = loadDatabase as jest.Mock;
const mockSaveDatabaseFunction = saveDatabase as jest.Mock;

// Mock uuid module before it's potentially used by the service
jest.mock('uuid', () => {
...
describe('issueService - Create Operations', () => {
  ...
  it('should create a Task with a parentKey pointing to an existing Epic', async () => {
    ...
    expect(savedDbState!.issues.length).toBe(2); // Existing Epic + New Task
    ...
    expect(createdIssue).toEqual(savedTask); // Ensure returned object matches saved object
  });
  ...
  it('should create a Task with a valid parentKey pointing to an existing Epic', async () => {
    const input = {
      title: 'Task with Epic Parent',
      description: 'This task has an Epic parent.',
      issueTypeName: 'Task',
  ...
  it('should throw IssueCreationError with code PARENT_NOT_FOUND (404) if parentKey does not exist', async () => {
    const input = {
      ...
      issueTypeName: 'Subtask',
      parentKey: 'NON-EXISTENT-123', // This key will not be in the mocked database
    };

    // Mock the database to return an empty state or a state without the specified parent
    const dbWithoutParent: DbSchema = {
        issues: [], // No issues, definitely no 'NON-EXISTENT-123'
        issueKeyCounter: 1,
    };

    mockLoadDatabaseFunction.mockResolvedValue(JSON.parse(JSON.stringify(dbWithoutParent))); // Corrected Deep copy

    // Assert that calling createIssue with the non-existent parentKey throws the expected error
    await expect(createIssue(input)).rejects.toThrow(IssueCreationError);

    // Verify the specific error code and HTTP status code
    try {
        await createIssue(input);
    } catch (error: any) { // Catch the error to check its properties
        expect(error).toBeInstanceOf(IssueCreationError);
        expect(error.code).toBe('PARENT_NOT_FOUND');
        expect(error.httpStatus).toBe(404);
        // Optionally, check the error message if the service provides one
        expect(error.message).toContain('Parent issue with key "NON-EXISTENT-123" not found');
    }

    // Verify database interactions - Load should happen, Save should NOT happen
    // Using expect(...).rejects requires calling the function, leading to two calls if not careful.
    // A single call check is sufficient for the expected behavior.
    expect(mockLoadDatabaseFunction).toHaveBeenCalledTimes(1); // Should load the db once to find the parent
    expect(mockSaveDatabaseFunction).not.toHaveBeenCalled(); // The operation should fail before saving
  });
  ...
  it('should throw IssueCreationError with code PARENT_NOT_FOUND (404) if parentKey does not exist', async () => {
    ...
    try {
        ...
        expect(error).toBeInstanceOf(IssueCreationError);
        expect(error.code).toBe('PARENT_NOT_FOUND');
        expect(error.httpStatus).toBe(404); // Assert the HTTP status code
        // Optionally, check the error message if the service provides one
        expect(error.message).toContain('Parent issue with key "NON-EXISTENT-123" not found');
    }

    // Verify database interactions - Load should happen once, Save should NOT happen
    expect(mockLoadDatabaseFunction).toHaveBeenCalledTimes(1); // Should load the db once to find the parent
    expect(mockSaveDatabaseFunction).not.toHaveBeenCalled(); // The operation should fail before saving
  });
});
