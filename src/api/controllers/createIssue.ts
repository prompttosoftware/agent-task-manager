...
export const createIssue = async (req: Request<{}, {}, IncomingCreateIssueRequestBody>, res: Response) => {
  ...
  try {
    ...
    if (!req.body || typeof req.body !== 'object' || Array.isArray(req.body)) {
        logger.error('IssueCreationError during validation: Invalid request body format (must be an object).', { error: IssueErrorCodes.INVALID_INPUT, statusCode: 400 });
        throw new IssueCreationError("Invalid request body format (must be a JSON object).", IssueErrorCodes.INVALID_INPUT, 400);
    }

    if (!('fields' in req.body) || typeof req.body.fields !== 'object' || req.body.fields === null || Array.isArray(req.body.fields)) {
         logger.error('IssueCreationError during validation: Missing or invalid "fields" object.', { error: IssueErrorCodes.INVALID_INPUT, statusCode: 400 });
        throw new IssueCreationError("Missing or invalid 'fields' object in request body.", IssueErrorCodes.INVALID_INPUT, 400);
    }
    ...
    if (!('summary' in fields) || typeof fields.summary !== 'string' || fields.summary.trim().length === 0) {
        logger.error('IssueCreationError during validation: Missing, invalid, or empty summary.', { error: IssueErrorCodes.MISSING_TITLE, statusCode: 400 });
         throw new IssueCreationError("Missing, invalid, or empty 'summary' in request fields.", IssueErrorCodes.MISSING_TITLE, 400);
    }

    logger.debug('createIssue Controller: Validating issuetype object and name field.');
    if (!('issuetype' in fields) || typeof fields.issuetype !== 'object' || fields.issuetype === null || Array.isArray(fields.issuetype)) {
         logger.error('IssueCreationError during validation: Missing or invalid "issuetype" object.', { error: IssueErrorCodes.INVALID_ISSUE_TYPE, statusCode: 400 }); // Using INVALID_ISSUE_TYPE for structure error
          throw new IssueCreationError("Missing or invalid 'issuetype' object in request fields.", IssueErrorCodes.INVALID_ISSUE_TYPE, 400);
     }

     if (!('name' in fields.issuetype) || typeof fields.issuetype.name !== 'string' || fields.issuetype.name.trim().length === 0) {
         logger.error('IssueCreationError during validation: Missing, invalid, or empty issuetype name.', { error: IssueErrorCodes.INVALID_ISSUE_TYPE, statusCode: 400 });
          throw new IssueCreationError("Missing, invalid, or empty 'issuetype.name' in request fields.", IssueErrorCodes.INVALID_ISSUE_TYPE, 400);
     }
    ...
    if ('description' in fields && typeof fields.description !== 'string') {
        logger.error('IssueCreationError during validation: Invalid description type (must be string if provided).', { error: IssueErrorCodes.INVALID_INPUT, statusCode: 400 });
        throw new IssueCreationError("Invalid 'description' type in request fields (must be a string if provided).", IssueErrorCodes.INVALID_INPUT, 400);
    }
    ...
    if ('parent' in fields) {
        if (typeof fields.parent !== 'object' || fields.parent === null || Array.isArray(fields.parent)) {
            logger.error('IssueCreationError during validation: Invalid "parent" object.', { error: IssueErrorCodes.INVALID_INPUT, statusCode: 400 });
        ...
        if (!('key' in fields.parent) || typeof fields.parent.key !== 'string' || fields.parent.key.trim().length === 0) {
             logger.error('IssueCreationError during validation: Missing, invalid, or empty parent key within "parent" object.', { error: IssueErrorCodes.INVALID_INPUT, statusCode: 400 }); // Using INVALID_INPUT for key structure/presence
             throw new IssueCreationError("Missing, invalid, or empty 'key' within 'parent' object.", IssueErrorCodes.INVALID_INPUT, 400);
        }
    ...
    if ((fields.issuetype.name === 'Epic' || fields.issuetype.name === 'Bug') && sanitizedParentKey) {
        ...
        logger.error('IssueCreationError during validation: Epic/Bug cannot have a parent.', { error: IssueErrorCodes.INVALID_PARENT_KEY, issueType: fields.issuetype.name, parentKey: sanitizedParentKey, originalParentKey: fields.parent?.key, statusCode: 400 }); // Log original for debugging
        throw new IssueCreationError("Epic and Bug issue types cannot have a parent issue.", IssueErrorCodes.INVALID_PARENT_KEY, 400, { issueType: fields.issuetype.name, parentKey: sanitizedParentKey, originalParentKey: fields.parent?.key }); // Pass details
    } else {
    ...
    if (sanitizedParentKey) {
      ...
      if (!ISSUE_KEY_REGEX.test(parentKey)) {
          // Re-adding log before throw to match test expectation
           logger.error('IssueCreationError during validation: Invalid parent key format.', { error: IssueErrorCodes.INVALID_INPUT, parentKey: parentKey, originalParentKey: fields.parent?.key, statusCode: 400 }); // Log original for debugging
          throw new IssueCreationError(`Parent issue key "${parentKey}" is invalid. Must be in format PROJECT-123.`, IssueErrorCodes.INVALID_INPUT, 400, { parentKey: parentKey, originalParentKey: fields.parent?.key }); // Pass details
      }
      ...
      if (!parentIssue) {
        // Re-adding log before throw to match test expectation
         logger.error('IssueCreationError during validation: Parent issue not found.', { error: IssueErrorCodes.PARENT_ISSUE_NOT_FOUND, parentKey: parentKey, originalParentKey: fields.parent?.key, statusCode: 404 }); // Log original for debugging
        throw new IssueCreationError(`Parent issue with key '${parentKey}' not found.`, IssueErrorCodes.PARENT_ISSUE_NOT_FOUND, 404, { parentKey: parentKey, originalParentKey: fields.parent?.key }); // Pass details
      }
      ...
      if (fields.issuetype.name === 'Subtask') {
          if (parentIssue.issueType !== 'Task' && parentIssue.issueType !== 'Story') {
            logger.error('IssueCreationError during validation: Invalid parent type for Subtask.', { error: IssueErrorCodes.INVALID_PARENT_TYPE, issueType: fields.issuetype.name, parentKey: parentKey, parentType: parentIssue.issueType, originalParentKey: fields.parent?.key, statusCode: 400 }); // Log original for debugging
            throw new IssueCreationError(`Parent issue with key '${parentKey}' must be of type 'Task' or 'Story' for a Subtask.`, IssueErrorCodes.INVALID_PARENT_TYPE, 400, { issueType: fields.issuetype.name, parentKey: parentKey, parentType: parentIssue.issueType, originalParentKey: fields.parent?.key }); // Pass details
          }
      ...
      if (fields.issuetype.name === 'Task' || fields.issuetype.name === 'Story') {
          if (parentIssue.issueType !== 'Epic') {
            logger.error('IssueCreationError during validation: Invalid parent type for Task/Story.', { error: IssueErrorCodes.INVALID_PARENT_TYPE, issueType: fields.issuetype.name, parentKey: parentKey, parentType: parentIssue.issueType, originalParentKey: fields.parent?.key, statusCode: 400 }); // Log original for debugging
            throw new IssueCreationError(`Parent issue with key '${parentKey}' must be of type 'Epic' for a Task or Story.`, IssueErrorCodes.INVALID_PARENT_TYPE, 400, { issueType: fields.issuetype.name, parentKey: parentKey, parentType: parentIssue.issueType, originalParentKey: fields.parent?.key }); // Pass details
          }
    ...
    if (error instanceof IssueCreationError) {
      logger.error("Caught IssueCreationError:", { error: error.message, code: error.errorCode, status: error.statusCode, details: error.details }); // Log details
