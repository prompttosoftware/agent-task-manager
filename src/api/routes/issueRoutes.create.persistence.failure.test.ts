...
describe('POST /rest/api/2/issues - Persistence Verification (Failure Cases)', () => {
  // Updated describe block name for clarity
  ...
  it('should return 400 and not persist the Subtask issue when parent is a Bug', async () => {
    ...
    expect(Array.isArray(response.body.errorMessages)).toBe(true); // Ensure it's an array
    // FIX: Updated assertion to check if the errorMessages array contains the specific message
    expect(response.body.errorMessages).toContain(
      `Issue with key '${bugKey}' has type 'Bug', which cannot be a parent of a Subtask. Only Epic or Story issues can be parents of Subtasks.`,
    ); // Specific error message check
  ...
  it('should return 400 and not persist the second Subtask issue when parent is another Subtask', async () => {
    ...
    // FIX: Updated assertion to check if the errorMessages array contains the specific message
    expect(response.body.errorMessages).toContain(
      `Issue with key '${firstSubtaskKey}' has type 'Subtask', which cannot be a parent of a Subtask. Only Epic or Story issues can be parents of Subtasks.`,
    ); // Specific error message check
    ...
    const persistedSecondSubtask = dbData.issues.find(
      (issue: AnyIssue) => issue.summary === 'Second Subtask under First Subtask (Invalid Parent)',
    );
