...
describe('BoardService', () => {
  ...
  beforeEach(async () => {
    ...
    boardRepository = module.get(BoardRepository) as MockBoardRepository;
    configService = module.get(ConfigService) as MockConfigService;
  });
  ...
  it('should create a board', async () => {
    ...
    boardRepository.createBoard.mockResolvedValue(createdBoard);
  ...
  it('should get a board by id', async () => {
    ...
    boardRepository.getBoardById.mockResolvedValue(board);
  ...
  it('should get all boards', async () => {
    ...
    boardRepository.getAllBoards.mockResolvedValue(boards);
  ...
  it('should update a board', async () => {
    ...
    boardRepository.updateBoard.mockResolvedValue(updatedBoard);
  ...
  it('should delete a board', async () => {
    const boardId: number = 1;
    boardRepository.deleteBoard.mockResolvedValue(undefined);
  ...
  it('should throw NotFoundException if board is not found', async () => {
    const boardId: number = 999;
    const errorMessage = `Board with ID ${boardId} not found`;
    boardRepository.getBoardById.mockResolvedValue(undefined);
