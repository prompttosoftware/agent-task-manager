// src/services/databaseService.rollbackTransaction.spec.ts
...
describe('DatabaseService.rollbackTransaction', () => {
    ...
    it('should call db.run with ROLLBACK', async () => {
        // Arrange
        const expectedSql = 'ROLLBACK';
    ...
    it('should handle errors from db.run', async () => {
        ...
        // Act & Assert
        await expect(databaseService.rollbackTransaction()).rejects.toThrow(expectedError);
        expect(mockGetDBConnection).toHaveBeenCalledTimes(1);
        expect(mockDb.run).toHaveBeenCalledTimes(1);
    });
});